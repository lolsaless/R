...
## Plotting
ggplot(df, aes(x=row_num, y=Transformed_Log_Ca, color=Sample)) +
    geom_point() +
    geom_hline(yintercept=0, linetype="dashed", color = "red") +
    labs(title="Transformed Log Ca Values by Sample", x="Index", y="Transformed_Log_Ca") +
    theme_minimal()
...
ggplot(df, aes(x=row_num, y=Transformed_Log_Ca, color=density, shape=Sample)) +
    ...
    geom_hline(yintercept=0, linetype="dashed", color = "red") +
    labs(title="Transformed Log Ca Values by Sample with Den sity", x="Index", y="Transformed_Log_Ca") +
    theme_minimal() +
...
p <- ggplot(df, aes(x=row_num, y=Transformed_Log_Ca, color=density_cat, shape=Sample)) +
    ...
    labs(title="Transformed Log Ca Values by Sample with Density", x="Index", y="Transformed_Log_Ca") +
    theme_minimal() +
...
p + geom_text_repel(
    ...
    aes(label = name_new, color = NULL, Shape = NULL),  # name_new 추가
...
p <- ggplot(df, aes(x=row_num, y=Transformed_Log_Ca, color=density_cat, shape=Sample)) +
    ...
    geom_hline(yintercept=0, linetype="dashed", color = "red") +
    geom_hline(yintercept=c(0.5, -0.5), linetype="dashed", color = "#4682B4") +
    labs(title="Transformed Log  Values by Part with Density", x="Index", y="Transformed_Log_") +
    theme_minimal() +
    scale_colour_manual(values = c("Low" = "#2E8B57", "Medium" = "#FF8C00", "High" = "#D2691E", "Very High" = "#DC143C"))
